...
//    ws: null
  },
  {
     ...
     args:[
       ...
       "-i",  "/dev/video0" ,
       "-framerate", 30,
       "-video_size", 640 + 'x' + 480,
//       '-vf',  'crop=640:480:650:200',
       '-pix_fmt',  'yuv420p',
       '-c:v',  'libx264',
////     '-b:v', '600k',
////     '-bufsize', '600k',
...
streamList.forEach((stream) => {
  try {
    ...
    function createStream() {
      ...
      console.log("ffmpeg " + stream.args.join(' '));
      let cmd = spawn('./ffmpeg', stream.args, {
        detached: true,
      ...
      ffmpeg.on("data", (data)=> {
        ...
        const arr = new Uint8Array(data);
        //log.info('[recv.NAL]', arr);

        if( arr[0] === 103 ) {
          log.info('[recv.iframe] ');
          frameQueue = [];
          stream.frameQueue = frameQueue;
