...
class Lobby extends Network {
  ...
  constructor (config, roomName) {
    ...
    this.sendGameCommand = (cmd) => {
      let packet = cmd.packet;
      if (typeof packet.data === 'object') {
        packet.data = JSON.stringify(packet.data);
      }
      let data = {
        gid: cmd.gid,
        packet: cmd.packet
...
export { base as GameBase, Lobby, client, components, game, init, scene, ui, updateManager };
