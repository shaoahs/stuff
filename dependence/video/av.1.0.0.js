...
LCS.report = null;
LCS.labelName = 'unknown'

LCS.recordVideo = {
    name: '視訊模組',
    category: '使用的頻道',
    getLabel: () => LCS.labelName
  };

LCS.setReport = function (report) {
    LCS.report = report;
}
...
LCS.CreateVideoElements = function() {
    var elements = document.querySelectorAll(".LCS");
    for (var i = 0; i < elements.length; i++) {
        new LCS.VideoElement(elements[i]);
    }
};

LCS.Fill = function(array, value) {
    if (array.fill) {
        array.fill(value);
    } else {
...
LCS.Player = function() {
    "use strict";
    var Player = function ClassPlayer(url, options) {
        if(LCS.report) {
            LCS.recordVideo.category = `使用頻道 ${url}`;
            LCS.report.serviceBegin(LCS.recordVideo);
        }
        this.isFirstTimeDecode = true;
        this.sec = 0;
        this.DisplayURL = url;
        ...
        if (options.source) {
            ...
            this.source = new LCS.Source.AjaxProgressive(url, options);
            options.streaming = false;
        } else {
            this.source = new LCS.Source.Ajax(url, options);
            options.streaming = false;
        }
        ...
        if (options.video !== false) {
            this.video = new LCS.Decoder.MPEG1Video(options);
        ...
        if (options.audio !== false) {
            this.audio = new LCS.Decoder.MP2Audio(options);
    ...
    Player.prototype.updateFPS = function() {};
    Player.prototype.updateForStreaming = function() {
        if (this.video) {
            ...
            if (this.sec != d.getSeconds()) {
                ...
                if (this.FPS < 20) {
                    console.log(d.toString() + "(" + this.DisplayURL + ")FPS==" + this.FPS);
                }
                if (this.Debug == true) {
                    var Bytes = (LCS.Bandwidth / 1024).toFixed(3);
                    document.title = "FPS:" + this.FPS + "  ,   " + Bytes.toString() + "KB     (2019/2/27 version)";
                    console.log("receive:" + (LCS.Bandwidth / 1024).toFixed(3) + " KB");
                }
                LCS.Bandwidth = 0;
                this.countFrame = this.video.currentFrame;
            }
            if(this.isFirstTimeDecode) {
                this.isFirstTimeDecode = false;
                if(LCS.report) {
                    LCS.report.serviceEnd(LCS.recordVideo);
                }
            }
        }
        if (this.audio) {
...
LCS.Source.AjaxProgressive = function() {
    "use strict";
    ...
    AjaxProgressiveSource.prototype.onChunkLoad = function(data) {
        ...
        this.loadTime = LCS.Now() - this.loadStartTime;
        if (!this.throttled) {
            this.loadNextChunk();
        }
    };
...
LCS.Source.WebSocket = function() {
    ...
    WSSource.prototype.onMessage = function(ev) {
        if (this.destination) {
            ...
            var bytes = ev.data.byteLength;
            LCS.Bandwidth = LCS.Bandwidth + bytes;
        }
    };
    return WSSource;
}();
...
LCS.Decoder.MPEG1Video = function() {
    ...
    MPEG1.prototype.currentFrame = 0;
...
LCS.AudioOutput.WebAudio = function() {
    ...
    var WebAudioOut = function(options) {
        ...
        this.volume = 1;
        this.enabled = true;
        this.unlocked = true//!WebAudioOut.NeedsUnlocking();
