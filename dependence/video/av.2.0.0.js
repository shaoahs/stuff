var LCS = {
    ...
    //     var elements = document.querySelectorAll(".mpeg");
    //     for (var i = 0; i < elements.length; i++) {
    //         new LCS.VideoElement(elements[i]);
    //     }
    // },
...
//     LCS.CreateVideoElements();
// } else {
//     document.addEventListener("DOMContentLoaded", LCS.CreateVideoElements);
// }

// LCS.VideoElement = function() {
//     "use strict";
...
//             }
//         }
//         this.player = new LCS.Player(url, options);
...
LCS.Player = function() {
    ...
    var Player = function(url, options) {
        ...
        if (options.source) {
            ...
            this.source = new LCS.Source.WebSocket(url, options);
            options.streaming = true;
        }
        this.maxAudioLag = options.maxAudioLag || .25;
        this.loop = options.loop !== false;
        this.autoplay = !!options.autoplay || options.streaming;
        this.demuxer = new LCS.Demuxer.TS(options);
        this.source.connect(this.demuxer);
        if (!options.disableWebAssembly && LCS.WASMModule.IsSupported()) {
            this.wasmModule = LCS.WASMModule.GetModule();
            options.wasmModule = this.wasmModule;
        }
        if (options.video !== false) {
            this.video = options.wasmModule ? new LCS.Decoder.MPEG1VideoWASM(options) : new LCS.Decoder.MPEG1Video(options);
            this.renderer = !options.disableGl && LCS.Renderer.WebGL.IsSupported() ? new LCS.Renderer.WebGL(options) : new LCS.Renderer.Canvas2D(options);
            this.demuxer.connect(LCS.Demuxer.TS.STREAM.VIDEO_1, this.video);
            this.video.connect(this.renderer);
        }
        if (options.audio !== false && LCS.AudioOutput.WebAudio.IsSupported()) {
            this.audio = options.wasmModule ? new LCS.Decoder.MP2AudioWASM(options) : new LCS.Decoder.MP2Audio(options);
            this.audioOut = new LCS.AudioOutput.WebAudio(options);
            this.demuxer.connect(LCS.Demuxer.TS.STREAM.AUDIO_1, this.audio);
        ...
        if (this.wasmModule) {
            if (this.wasmModule.ready) {
                this.startLoading();
            } else if (LCS.WASM_BINARY_INLINED) {
                var wasm = LCS.Base64ToArrayBuffer(LCS.WASM_BINARY_INLINED);
                this.wasmModule.loadFromBuffer(wasm, this.startLoading.bind(this));
            } else {
                this.wasmModule.loadFromFile("lcs.wasm", this.startLoading.bind(this));
            }
        } else {
            this.startLoading();
        }
    ...
    Player.prototype.update = function() {
        ...
        if (!this.isPlaying) {
            this.isPlaying = true;
            this.startTime = LCS.Now() - this.currentTime;
    ...
    Player.prototype.updateForStreaming = function() {
        if (this.video) {
            this.video.decode();
            var d = new Date();
            if (this.sec != d.getSeconds()) {
                this.sec = d.getSeconds();
                this.mill = d.getMilliseconds();
                this.FPS = this.video.currentFrame - this.countFrame;
                if (this.FPS < 20) {
                    console.log(d.toString() + this.FPS);
                }
                if (this.Debug == true) {
                ...
                this.countFrame = this.video.currentFrame;
            }
        }
        if (this.audio) {
            var decoded = false;
    ...
    Player.prototype.updateForStaticFile = function() {
        ...
        if (this.audio && this.audio.canPlay) {
            ...
            var targetTime = LCS.Now() - this.startTime + this.video.startTime, lateTime = targetTime - this.video.currentTime, frameTime = 1 / this.video.frameRate;
...
LCS.Demuxer.TS = function() {
    ...
    TS.prototype.write = function(buffer) {
        if (this.leftoverBytes) {
            ...
            this.bits = new LCS.BitBuffer(totalLength);
            this.bits.write([ this.leftoverBytes, buffer ]);
        } else {
            this.bits = new LCS.BitBuffer(buffer);
        }
    ...
    TS.prototype.resync = function() {
        ...
        console.warn("LCS: Possible garbage data. Skipping.");
...
LCS.Decoder.MPEG1Video = function() {
    "use strict";
    var MPEG1 = function(options) {
        LCS.Decoder.Base.call(this, options);
        this.onDecodeCallback = options.onVideoDecode;
        var bufferSize = options.videoBufferSize || 512 * 1024;
        var bufferMode = options.streaming ? LCS.BitBuffer.MODE.EVICT : LCS.BitBuffer.MODE.EXPAND;
        this.bits = new LCS.BitBuffer(bufferSize, bufferMode);
    ...
    MPEG1.prototype = Object.create(LCS.Decoder.Base.prototype);
    MPEG1.prototype.constructor = MPEG1;
    MPEG1.prototype.write = function(pts, buffers) {
        LCS.Decoder.Base.prototype.write.call(this, pts, buffers);
    ...
    MPEG1.prototype.decode = function() {
        ...
        var elapsedTime = LCS.Now() - startTime;
...
LCS.Decoder.MPEG1VideoWASM = function() {
    "use strict";
    var MPEG1WASM = function(options) {
        ...
        this.bufferMode = options.streaming ? LCS.BitBuffer.MODE.EVICT : LCS.BitBuffer.MODE.EXPAND;
        this.decodeFirstFrame = options.decodeFirstFrame !== false;
        this.onReady = options.onReady;
        this.hasSequenceHeader = false;
    };
    MPEG1WASM.prototype = Object.create(LCS.Decoder.Base.prototype);
    MPEG1WASM.prototype.currentFrame = 0;
    MPEG1WASM.prototype.constructor = MPEG1WASM;
    MPEG1WASM.prototype.initializeWasmDecoder = function() {
        if (!this.module.instance) {
            console.warn("LCS: WASM module not compiled yet");
            return;
        }
    ...
    MPEG1WASM.prototype.decode = function() {
        ...
        if (this.destination) {
            var ptrY = this.functions._mpeg1_decoder_get_y_ptr(this.decoder), ptrCr = this.functions._mpeg1_decoder_get_cr_ptr(this.decoder), ptrCb = this.functions._mpeg1_decoder_get_cb_ptr(this.decoder);
...
LCS.Decoder.MP2Audio = function() {
    ...
    MP2.prototype = Object.create(LCS.Decoder.Base.prototype);
...
LCS.Decoder.MP2AudioWASM = function() {
    "use strict";
    ...
    MP2WASM.prototype = Object.create(LCS.Decoder.Base.prototype);
    ...
    MP2WASM.prototype.decode = function() {
        ...
        if (this.destination) {
            var leftPtr = this.functions._mp2_decoder_get_left_channel_ptr(this.decoder), rightPtr = this.functions._mp2_decoder_get_right_channel_ptr(this.decoder);
            var leftOffset = leftPtr / Float32Array.BYTES_PER_ELEMENT, rightOffset = rightPtr / Float32Array.BYTES_PER_ELEMENT;
            var left = this.instance.heapF32.subarray(leftOffset, leftOffset + MP2WASM.SAMPLES_PER_FRAME), right = this.instance.heapF32.subarray(rightOffset, rightOffset + MP2WASM.SAMPLES_PER_FRAME);
...
LCS.Renderer.Canvas2D = function() {
    "use strict";
    var CanvasRenderer = function(options) {
        ...
        console.log("Renderer Canvas 2d");
    };
    ...
    CanvasRenderer.prototype.resize = function(width, height) {
        ...
        LCS.Fill(this.imageData.data, 255);
    };
...
LCS.AudioOutput.WebAudio = function() {
    ...
    WebAudioOut.prototype.play = function(sampleRate, left, right) {
        ...
        if (this.startTime < now) {
            this.startTime = now;
            this.wallclockStartTime = LCS.Now();
        }
    ...
    WebAudioOut.prototype.getEnqueuedTime = function() {
        return Math.max(this.wallclockStartTime - LCS.Now(), 0);
    };
...
LCS.WASMModule = function() {
    ...
    WASM.prototype.c_abort = function(size) {
        console.warn("LCS: WASM abort", arguments);
    };
    WASM.prototype.c_assertFail = function(size) {
        console.warn("LCS: WASM ___assert_fail", arguments);
    };
    WASM.prototype.readDylinkSection = function(buffer) {
        ...
        if (!matchNextBytes([ 0, "a", "s", "m" ])) {
            console.warn("LCS: WASM header not found");
            return null;
        }
        var next = 9;
        var sectionSize = readVarUint();
        if (!matchNextBytes([ 6, "d", "y", "l", "i", "n", "k" ])) {
            console.warn("LCS: No dylink section found in WASM");
            return null;
        }
        return {
