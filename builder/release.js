...
exports.build = function(workspace, done, grunt){
  ...
  let output = rootPath + workspace.current + '/release/'+ workspace.output + '.js';
  builder.buildStatic(workspace.current, output, set.build)
    .then(function(){
      done();
    })
    .catch(function(err){
